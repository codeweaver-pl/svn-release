...
var DEFAULT_LOG_MESSAGE = 'please provide a log message to maintain meaningful history';

function QSVN(config, callback) {

  this.svn            = new SVN(config, callback);
  this.defaultMessage = (config || {}).defaultMessage || DEFAULT_LOG_MESSAGE;
...
QSVN.prototype.commit = function (files, message) {
  return this.commit(files, message || this.defaultMessage);
};
...
QSVN.prototype.copy = function (source, target, message) {
  return this.copy(source, target, message || this.defaultMessage);
};
 ...
 * @returns {Promise}
 */
QSVN.prototype.info = function (command) {
...
/**
 * Subversion switch
 * @param {!string} url Url to update-switch to
 * @returns {Promise}
 */
